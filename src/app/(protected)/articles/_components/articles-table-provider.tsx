...
interface ArticlesTableContextProps {
    featureFlags: FeatureFlagValue[];
    setFeatureFlags: React.Dispatch<React.SetStateAction<FeatureFlagValue[]>>;
}

const ArticlesTableContext = React.createContext<ArticlesTableContextProps>({
    featureFlags: [],
    setFeatureFlags: () => {},
});

export function useArticlesTable() {
    const context = React.useContext(ArticlesTableContext);
    if (!context) {
        throw new Error(
            "useArticlesTable must be used within a ArticlesTableProvider"
        );
    }
    return context;
}

export function ArticlesTableProvider({ children }: React.PropsWithChildren) {
